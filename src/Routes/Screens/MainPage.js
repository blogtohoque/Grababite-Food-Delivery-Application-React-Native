...
class MainPage extends Component {
	...
	componentDidMount = async () => {
		...
		if (full_name === null) {
			...
			axios
				.post('http://192.168.140.2:8000/checkEmail', {
					name: x
				})
				.then((Response) => {
						...
						this.state.gotdata.map((print) => {
							return (
								<Card>
									<CardItem
										cardBody
										style={{ borderTopLeftRadius: 25, borderTopRightRadius: 25, borderWidth: 0 }}
									>
										<Image
											source={require('../../assets/images/fm.jpg')}
									...
									<CardItem>
										<View>
											<Text style={{ color: 'black', fontSize: 11 }}>
												{' '}
												Food From {print.name}'s Best Vendors.{' '}
											</Text>
										</View>
									</CardItem>
											...
											<View style={{ backgroundColor: 'rgb(179, 0, 0)', width: '100%' }}>
												...
												>
													<Text
														...
														onPress={() => {
															this.onViewClick(print.name);
															this.props.onCityAdd(print.name);
														}}
													>
														View
													</Text>
...
const styles = StyleSheet.create({
	tr: {
		flex: 1,
		transform: [ { translateX: -Dimensions.get('window').width * 0.24 }, { rotateY: '30deg' } ]
	},
	container: {
		flex: 1,
